/* =========================================================
   ...
   Talks only to /api/* (Netlify Functions), never to JSONBin.
   ========================================================= */
...
function switchTab(tabName) {
  ...
  if (tabName === 'upcoming') {
    ...
    proposeBtn.classList.add('text-purple-200');
    ...
    upcomingBtn.classList.add('text-purple-200');
...
async function loadSpins() {
  ...
  try {
    ...
    // Rebuild local RSVP map from server state + local user name
    userRSVPs = {};
    spinsData.forEach((s) => {
      if (currentUserName && s.committed.includes(currentUserName)) userRSVPs[s.id] = 'committed';
...
function renderSpinCard(spin) {
  ...
  const totalCommitted = spin.committed.length + (isCommitted ? 1 : 0);
  const totalInterested = spin.interested.length + (isInterested ? 1 : 0);
  ...
  const mapBtn = spin.mapLink
    ? `<a href="${spin.mapLink}" target="_blank" rel="noopener" class="inline-flex items-center px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-bold border border-slate-300 transition">
         <i class="fa-solid fa-map-location-dot text-clubBlue mr-1.5"></i> Route Map
       </a>`
    : '';

  const mudguardBadge = spin.mudguardsRequired
    ? `<span class="bg-slate-200 text-slate-800 text-xs font-bold px-2 py-1 rounded-md border border-slate-300" title="Please use mudguards on wet spins">
         <i class="fa-solid fa-shield-halved text-clubPurple mr-1"></i> Mudguards
       </span>`
    : '';
  ...
  const weatherBadge = spin.weatherPolicy === 'All-Weather'
    ? `<span class="bg-amber-100 text-amber-900 border border-amber-300 text-xs font-extrabold px-2.5 py-1 rounded-md">
         <i class="fa-solid fa-cloud-showers-heavy mr-1"></i> All-Weather
       </span>`
    : `<span class="bg-sky-100 text-sky-900 border border-sky-300 text-xs font-extrabold px-2.5 py-1 rounded-md">
         <i class="fa-solid fa-sun mr-1"></i> Fair-Weather Only
       </span>`;
  ...
  const quorumBadge = quorumMet
    ...
    : `<span class="bg-amber-100 text-amber-800 text-xs font-black px-2 py-0.5 rounded-full border border-amber-300"><i class="fa-solid fa-hourglass-half mr-1"></i> Needs ${spin.minRiders - totalCommitted} more rider(s)</span>`;

  const phoneDigits = String(spin.phone || '').replace(/[^0-9]/g, '');
  const waHref = `https://wa.me/${phoneDigits}?text=${encodeURIComponent(
    "Hi " + spin.author + ", I'm asking about the BrayCC spin: " + spin.title
  )}`;
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-md hover:shadow-lg transition overflow-hidden">
      <div class="bg-slate-100 px-4 py-3 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center space-x-2">
          <span class="bg-purple-900 text-white text-xs font-bold px-2.5 py-1 rounded-md flex items-center">
            <i class="${getSpinTypeIcon(spin.type)} mr-1.5"></i> ${spin.type}
          </span>
      ...
      <div class="p-5">
        ...
        <div class="mt-4 pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
          <div>
            <div class="flex items-center space-x-2">
              <span class="text-xs font-bold text-slate-500">Ride Status:</span>
              ${quorumBadge}
            </div>
            <div class="text-xs text-slate-600 mt-1">
              <span class="font-bold text-slate-800">${totalCommitted}</span> Committed &bull;
              <span class="font-bold text-slate-800">${totalInterested}</span> Interested
            </div>
          </div>
          ...
          <div class="flex items-center justify-between md:justify-end space-x-3">
            <div class="text-left md:text-right">
              ...
              <span class="text-xs font-extrabold text-slate-800">${escapeHtml(spin.author)}</span>
            </div>
        ...
        <div class="mt-4 bg-slate-50 p-3 rounded-lg border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div class="flex items-center space-x-2 w-full sm:w-auto">
            <button onclick="toggleRSVP('${spin.id}', 'committed')"
              ...
              <i class="fa-solid fa-circle-check"></i><span>Committed (Turning Up)</span>
            </button>
            <button onclick="toggleRSVP('${spin.id}', 'interested')"
              ...
              <i class="fa-solid fa-star"></i><span>Interested</span>
            </button>
          </div>
        </div>
      </div>
...
function selectPace(paceColor) {
  ['Red', 'Orange', 'Yellow', 'Green', 'Blue'].forEach((p) => {
    document.getElementById('pace-opt-' + p).classList.remove('selected');
  });
  document.getElementById('pace-opt-' + paceColor).classList.add('selected');
...
async function registerServiceWorker() {
  ...
  try {
    ...
    if (existing) {
      ...
      const btn = document.getElementById('enable-notifications');
      if (btn) {
        btn.innerHTML = '<i class="fa-solid fa-bell"></i><span>Alerts On</span>';
...
async function enablePush() {
  try {
    ...
    alert('Could not enable alerts: ' + err.message);
  }
}
